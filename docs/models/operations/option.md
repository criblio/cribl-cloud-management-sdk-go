...
```go
criblcloudmanagementsdkgo.WithServerURL("https://api.example.com")
```
...
```go
criblcloudmanagementsdkgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
criblcloudmanagementsdkgo.WithServerIndex(1)
```
...
```go
criblcloudmanagementsdkgo.WithClient(httpClient)
```
...
```go
criblcloudmanagementsdkgo.WithSecurity(/* ... */)
```
...
```go
criblcloudmanagementsdkgo.WithSecuritySource(/* ... */)
```
...
```go
criblcloudmanagementsdkgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
criblcloudmanagementsdkgo.WithTimeout(30 * time.Second)
```
